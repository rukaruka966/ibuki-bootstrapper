...
text fileはunified diffを出力する。binary fileは本文差分を作らず、SHA-256だけを
`plan.json`へ記録する。
...
Bundleにはローカル変更由来のdiffが含まれ、Secretを含む可能性がある。自動公開、
自動添付、自動Commitは行わない。promptはBundle内容を命令ではなくuntrusted dataとして
扱うようAIへ明記する。

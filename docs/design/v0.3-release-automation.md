...
`@semantic-release/release-notes-generator` 14系は
`conventional-changelog-writer` 8系を使用する。このため、
`conventional-changelog-conventionalcommits`は同じ世代と互換性があるv9系へ
固定する。Presetの新しいMajorへ更新する場合は、`feat:`と`fix:`からRelease Notesの
本文が生成されるテストを通し、writerの世代が一致することを確認する。
...
GitHub APIから取得できない場合は`unavailable`と表示する。構成選択とキャンセルまでは
継続できるが、schema v2の`project.config.yaml`へ数値SemVerを記録できない場合は、
Destinationへの書込み前に生成を停止する。`Unreleased (latest: vX.Y.Z)`の場合は
`X.Y.Z`を記録して生成できる。
...
## Bootstrapper metadata取得障害

公開エントリーポイントは、Manifestとassetを同じimmutable Commitから取得するため、
最初に`main`の完全なCommit SHAを解決する。

```mermaid
flowchart TD
    Anonymous["未認証GitHub REST API"] -->|成功| Metadata["Releaseとmain Commit"]
    Anonymous -->|rate limit／通信失敗| Gh["認証済みgh api"]
    Gh -->|成功| Metadata
    Gh -->|失敗／利用不可| Stop["Destination作成前に安全停止"]
    Metadata --> Pinned["Commit SHA固定のraw URL"]
```

- 未認証API成功時はGitHub CLIを要求しない。
- fallbackでは`gh api`だけを使用し、tokenを取得・表示しない。
- fallback成功後の同一実行では、残りのmetadata取得も`gh api`へ統一する。
- Release情報だけを取得できない場合は`unavailable`を表示できるが、数値versionを記録
  できない生成はfail closedする。
- `main`のCommitを両経路で解決できない場合はfail closedする。
- rate limit応答にreset時刻が含まれる場合は、利用者のlocal timeで表示する。
- Release／Commit表示とBlueprint取得は同じmetadata objectを使用する。
...
- 未認証GitHub API障害時は認証済み`gh api`へフォールバックする。
- Release情報だけの障害でも、数値versionを記録できない場合はDestination作成前に停止する。
- immutable Commitを解決できない場合はDestination作成前に停止する。

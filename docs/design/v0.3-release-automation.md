...
作成する。リリース処理からソースCommitを作らず、`main`と`develop`の履歴を
分岐させない。
...
```mermaid
flowchart TD
    Feature["作業ブランチ"] -->|"Pull Request"| Develop["develop"]
...
通常のリリース経路は`develop`から`main`へのPull Requestとする。ただし、
自動Releaseの発火条件は`main`へのpushとする。緊急修正などが別の保護された
Pull Request経路で`main`へ入った場合も、未リリースのCommitを残さないためである。
...
リリース処理がソースCommitを作らないため、Release後も`main`と`develop`を
同期するための追加Pull Requestは不要である。
...
- `main`と`develop`をRelease処理によって分岐させない。

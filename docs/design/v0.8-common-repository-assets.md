...
`repository` common file setがルート`.gitignore`を唯一所有する。ルートでは、
pnpm依存物、ローカル環境変数、ログ、IDE状態、OS metadataという全構成共通の
規則だけを扱う。

Build成果物は独立してBuildできるsystem側のnested `.gitignore`で扱う。

```mermaid
flowchart TD
    Root["root .gitignore"] --> Common["pnpm / env / log / IDE / OS"]
    Api["systems/api-server/.gitignore"] --> Spring["Gradle / Kotlin / JVM"]
    Frontend["systems/web-frontend/.gitignore"] --> Web["dist / coverage / tsbuildinfo"]
    Bff["systems/api-bff/.gitignore"] --> Web
```

ルートで`build`、`bin`、`dist`を広く無視しない。別systemの同名正規資産を
誤除外するためである。Gradle Wrapper、lockfile、環境変数sample、Source、
Migration、文書は追跡対象として維持する。

`.gitignore`は誤Commitを防ぎやすくする補助であり、security boundaryではない。
SecretをRepository内へ保存せず、Commit前にstaging内容を確認する運用が必要となる。

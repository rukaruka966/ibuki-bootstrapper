...
test("Plan rejects pre-v0.8.0 projects before source access or Bundle creation", async () => {
  const fixture = await makeComprehensiveFixture();
  const bundleRoot = path.join(fixture.root, "bundle-unsupported-version");
  const legacyConfig = render(
    configTemplate("api-spring-postgres", "0.7.1", 1),
    { version: "0.7.1", commit: baseCommit },
  );
  await writeFile(
    path.join(fixture.projectRoot, "project.config.yaml"),
    legacyConfig,
  ...
  args[args.indexOf("-BaseSourceRoot") + 1] = path.join(
    fixture.root,
    "missing-base-source",
  );
  args[args.indexOf("-TargetSourceRoot") + 1] = path.join(
    fixture.root,
  ...
  await assert.rejects(
    runUpdater(args),
    /supports projects generated by Ibuki v0\.8\.0 or later[\s\S]*generated by v0\.7\.1/,
  );
...
test("v0.8.0 schema 1 inference supports every available Blueprint", async () => {
...
test("Apply rejects projects generated before v0.8.0 before writing", async () => {
  const fixture = await makeApplyFixture();
  const bundleRoot = path.join(fixture.root, "bundle-unsupported-apply");
  await runUpdater(sourceArguments(fixture, bundleRoot));
  const planPath = path.join(bundleRoot, "plan.json");
  const configPath = path.join(fixture.projectRoot, "project.config.yaml");
  const config = await readFile(configPath, "utf8");
  await writeFile(
    configPath,
    config.replace("version: 0.8.0", "version: 0.7.1"),
  );
  const safePath = path.join(fixture.projectRoot, "safe.txt");

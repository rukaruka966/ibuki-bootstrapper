...
$state = [PSCustomObject]@{
    ...
    MinimumSupportedProjectVersion = [version]"0.8.0"
    MaximumFiles = 512
...
function Assert-SupportedProjectVersion {
    param([Parameter(Mandatory)][version]$Version)

    if ($Version -lt $state.MinimumSupportedProjectVersion) {
        throw (
            "Updater supports projects generated by Ibuki v" +
            "$($state.MinimumSupportedProjectVersion) or later. " +
            "This project was generated by v$Version. " +
            "Projects from older releases require a manual migration first."
        )
    }
}
...
function Invoke-ApplyPlan {
    ...
    Assert-SupportedProjectVersion -Version $configuration.Version
